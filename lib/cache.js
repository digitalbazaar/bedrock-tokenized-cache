...
export async function get({id, tokenizedId, tokenizer, explain = false} = {}) {
  ...
  if(tokenizedId === undefined) {
    ...
    // tokenize ID
    ({tokenizedId, tokenizer} = await _tokenizeId({id, tokenizer}));
  }
...
/**
 * Tokenizes the given ID. Exported for testing purposes only.
 *
 * @param {object} options - Options to use.
 ...
 */
export async function _tokenizeId({id, tokenizer} = {}) {
  assert.string(id, 'id');
  assert.optionalObject(tokenizer, 'tokenizer');
